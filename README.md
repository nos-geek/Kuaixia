...
- 包名：`com.kuaixia.app`
- 当前版本：**v1.1.0**（支持 Android 8.0 及以上）
...
- **抖音高清解析**：优先通过 PC 形态页面获取**多档清晰度**（最高可到 2160P/4K，具体档位以作品与设备的实际返回为准），可在清晰度列表中自行选择；未命中时**自动回退**到原有解析链路，不影响其它平台。
...
| 平台 | 说明 |
|------|------|
| 抖音 | 视频与图集；需网页会话（App 内可登录获取 Cookie）；支持多档清晰度选择（最高可到 2160P/4K，视作品与设备实际返回） |
| YouTube | 本地解析（yt-dlp） |
...
> **档位标识说明**：快夏使用视频**实际高度**作为清晰度档位标识，例如竖屏 2160×3840 的视频显示为 `3840P`（横屏 1920×1080 的视频显示为 `1080P`）。下载得到的文件分辨率与所选档位一致。数据来自平台页面实际返回的档位，实际可选档位因作品而异。
...
- **v1.1.0（当前）**：抖音解析新增 **PC 形态页面的高清链路**——优先获取多档清晰度（最高可到 2160P/4K，视作品与设备实际返回），未命中时自动回退到原有链路，解析稳定性与回退可靠性同步优化。稳定性说明：该链路已通过连续解析测试（同一作品重复解析结果一致），下载文件分辨率与所选档位一致。其余功能与 v1.0.0 相同。
- **v1.0.0**：首个正式版本。包含链接解析（本地 / 服务器 / 混合）、下载（直链 / M3U8 / 图集 / 音视频合并）、下载任务管理与通知、剪贴板与系统分享入口、解析服务器管理、深浅主题、简体中文 / English 双语、开发者诊断页。
- 各版本详细变更见 [`CHANGELOG.md`](CHANGELOG.md)；后续版本的规划与变更将随开发进展在此更新（历史阶段划分不再单独维护）。
